// Intent name: Departure
const fetch = require("node-fetch");
//Link to fetch data from the departure location Rotterdam

var link = encodeURI('http://api.openweathermap.org/data/2.5/weather?q=Rotterdam&units=Metric&appid=e033d33be97df555c5734d85d49b460a')
...
async function fetchData(){
    ...
    const temperature = await data.main.temp
        return "The temperature from the departure location Rotterdam is " 
        + temperature 
...
fetchData().then(function(result: any){
    array.push(result)
    })
 

export const departure = (conv: any) => {
